"""S5 identity test via permutation composition."""
    ...
    for token in tokens:
        ...
        if token.startswith('s') and token[1:].isdigit():
            ...
            if idx in VALID_INDICES:
                ...
                raise ValueError(f"Invalid generator index: {token} (must be s1-s4)")
        else:
...
def run_tests():
    """Run tests."""
    print("Running S5 identity test cases...")
    print("=" * 60)
    ...
    required_tests = [
        ...
        ("s1 s2 s3", False, "s1 s2 s3 != identity"),
    ]
    ...
    additional_tests = [
        ...
        ("s1 s2 s3 s4", False, "Long cycle"),
        ("s4 s3 s2 s1 s1 s2 s3 s4", True, "w * w^-1 pattern"),
        ("s1 s3", False, "Product of distant transpositions"),
    ]
    ...
    try:
        assert test_identity("s1 s1") == True
        assert test_identity("s2 s2") == True
        assert test_identity("s1 s2 s1 s2 s1 s2") == True  # Braid
        assert test_identity("s1") == False
        assert test_identity("s1 s2") == False
        # Additional assertions for commutation
        assert test_identity("s1 s3") == test_identity("s3 s1")  # Commute check
...
def demo_permutations():
    ...
    print("\nGenerator Actions on [1,2,3,4,5]:")
    print("-" * 40)
    for gen in ALPHABET:
        perm = evaluate(gen)
